...
df$X <- NULL
df$X1 <- NULL
na.omit(df) #RF cannot work on any NA
```
...
```{r}
num.vars = c("MSSubClass", "LotArea", "OverallQual", "OverallCond", "YearBuilt", "YearRemodAdd", "FirstFlrSF", "SecondFlrSF", "BsmtFullBath", "FullBath", "Fireplaces", "GarageArea", "GarageQual", "WoodDeckSF", "ScreenPorch", "PoolArea")
...
rf.form.all = as.formula("SalePrice ~ .")
...
rf.form.key <- as.formula(paste("SalePrice", varNames, sep = "~"))
```

Plot decision trees
```{r}
train.rf <- randomForest(rf.form.key,
                              train,
                              ntree=50,
                              importance=T)
